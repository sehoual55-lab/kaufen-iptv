...
(function () {
  "use strict";
  if (window.__kaCheckoutLoaded) return;
  window.__kaCheckoutLoaded = true;

  var CONFIG = {
    ENDPOINT: "https://script.google.com/macros/s/AKfycbzvoEBKDbLcXCxXGbGNX88wh3S7tFo8GcYScuqTegGU_om63tu5JIQTeTeBe-AVxcK58Q/exec",
  ...
  // -------- i18n --------
  var LANGS = {
    en: { eyebrow:"Kaufen IPTV Subscription", title:"Complete your order", sub:"Enter your details to activate your subscription.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Hi Kaufen IPTV! I'd like to order the "+p+" ("+d+", "+c+") for "+pr+". Name: "+n+". Payment: "+m+(l?(". Payment link: "+l):".");} },
    fr: { eyebrow:"Abonnement Kaufen IPTV", title:"Finalisez votre commande", sub:"Entrez vos informations pour activer votre abonnement.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Bonjour Kaufen IPTV, je souhaite commander "+p+" ("+d+", "+c+") pour "+pr+". Nom : "+n+". Paiement : "+m+(l?(". Lien de paiement : "+l):".");} },
    it: { eyebrow:"Abbonamento Kaufen IPTV", title:"Completa il tuo ordine", sub:"Inserisci i tuoi dati per attivare l'abbonamento.",
      ...
      msg:function(p,d,c,pr,n,m,l){return "Ciao Kaufen IPTV! Vorrei ordinare "+p+" ("+d+", "+c+") per "+pr+". Nome: "+n+". Pagamento: "+m+(l?(". Link di pagamento: "+l):".");} }
  };
  function detectLang(){ var p=location.pathname; if(/^\/fr(\/|$)/.test(p))return "fr"; if(/^\/it(\/|$)/.test(p))return "it";
    var l=(document.documentElement.lang||"").slice(0,2); return LANGS[l]?l:"de"; }
  LANGS.de = { eyebrow:"Kaufen IPTV Abonnement", title:"Bestellung abschließen", sub:"Geben Sie Ihre Daten ein, um Ihr Abonnement zu aktivieren.",
    name:"Vollständiger Name", namePh:"Max Mustermann", phone:"Telefonnummer", email:"E-Mail-Adresse", emailPh:"sie@beispiel.de",
    pay:"Zahlungsmethode", conn:"Gleichzeitige Verbindungen", cU1:"Verbindung", cU:"Verbindungen", duration:"Laufzeit", bonus:"Bonus",
    total:"Gesamt", submit:"Bestellung abschließen", secure:"Ihre Daten werden sicher übertragen.", guarantee:"7 Tage Geld-zurück-Garantie.",
    months:"Monate", free:"Monate gratis", okTitle:"Bestellung erhalten!", okBody:"Vielen Dank! Klicken Sie auf \u201eJetzt bezahlen\u201c, um die Zahlung abzuschließen, oder bestätigen Sie über WhatsApp.",
    paynow:"💳 Jetzt bezahlen", wa:"Über WhatsApp bestätigen", again:"Weitere Bestellung aufgeben",
    eName:"Bitte geben Sie Ihren Namen ein.", ePhone:"Ungültige Telefonnummer.", eEmail:"Ungültige E-Mail.", ePay:"Wählen Sie eine Zahlungsmethode.",
    msg:function(p,d,c,pr,n,m,l){return "Hallo Kaufen IPTV! Ich möchte das "+p+" Paket ("+d+", "+c+") für "+pr+" bestellen. Name: "+n+". Zahlung: "+m+(l?(". Zahlungslink: "+l):".");} };
  ...
  '.fs-sum{padding:30px 28px;background:radial-gradient(120% 70% at 6% 0%,rgba(232,199,102,.14),transparent 60%),#0a0c10;border-right:1px solid #171b22;display:flex;flex-direction:column}'+
  ...
  '.fs-pay:hover{border-color:#333b47}.fs-pay.sel{border-color:'+LIME+';background:rgba(232,199,102,.10);color:#fff}'+
  ...
  '.fs-check{width:66px;height:66px;border-radius:50%;background:rgba(232,199,102,.15);display:flex;align-items:center;justify-content:center;margin:0 auto 16px}'+
    ...
    modal.innerHTML=
      ...
      '<div class="fs-body" style="display:contents">'+
        '<aside class="fs-sum">'+
          ...
          '<div class="fs-badge" data-pop style="display:none">★ '+(LANG==="fr"?"Le Plus Populaire":LANG==="it"?"Più Popolare":LANG==="de"?"Am beliebtesten":"Most Popular")+'</div>'+
  ...
  function plansSection(){ return document.querySelector('#preise,#pricing,[id*="preis"],[id*="pricing"],[id*="plan"]'); }
  document.addEventListener("click",function(e){
    var a=e.target && e.target.closest ? e.target.closest('a[href*="wa.me"]') : null;
    if(!a) return;
    if(a.closest(".fs-modal")) return;                 // popup's own WhatsApp/pay buttons: leave alone
    var key=planFromEl(a);
    if(key && CONFIG.PLANS[key]){ e.preventDefault(); e.stopImmediatePropagation(); open(key); return; }
    var sec=plansSection();                             // any other WhatsApp button -> go to plans
