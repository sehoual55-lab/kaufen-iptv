    ...
    <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-charcoal-900/60 p-2">
      ...
      <div className="text-center">
        ...
        <span className="ml-1.5 text-xs text-slate-400">
          {value === 1 ? "Verbindung" : "Verbindungen"}
        </span>
      </div>
      <button
        type="button"
        ...
        aria-label="Verbindung hinzufügen"
...
function PlanCard({ plan, index }: { plan: Plan; index: number }) {
  ...
  const isPopular = plan.badge === "Am beliebtesten";
  const isValue = plan.badge === "Bester Wert";

  const orderMessage = `Hallo! Ich möchte das ${plan.name} Paket (${plan.duration}${
    plan.bonus ? " " + plan.bonus : ""
  }) mit ${connections} ${connections === 1 ? "Verbindung" : "Verbindungen"} für ${formatPrice(
    total,
    siteConfig.locale.currencySymbol
  )} kaufen. Bitte um die nächsten Schritte.`;
    ...
    >
        ...
        {hasDiscount && (
          <>
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald/10 px-2.5 py-0.5 text-xs font-medium text-emerald">
              −15 % · sparst {formatPrice(savings, siteConfig.locale.currencySymbol)}
            </span>
          </>
        )}
      </div>
      ...
      <div className="mb-5">
        ...
        <p className="mt-2 text-center text-xs text-slate-500">
          Erste Verbindung zum Normalpreis · jede weitere <span className="text-emerald">15 % günstiger</span>
        </p>
      </div>
      ...
      <div className="container relative">
        {showHeading && (
          <div className="mx-auto mb-14 max-w-3xl text-center">
            <span className="eyebrow">Preise &amp; Pakete</span>
            <h2 className="mt-5 font-display text-3xl font-bold tracking-tight text-white text-balance sm:text-4xl md:text-5xl">
              Wähle dein Paket und <span className="text-gold-gradient">kaufe dein IPTV</span>
            </h2>
            <p className="mt-5 text-lg text-slate-400">
              Transparente Preise, keine versteckten Kosten. Mit dem + / − Regler
              bestimmst du die Anzahl gleichzeitiger Verbindungen – der Preis passt
              sich automatisch an.
            </p>
          </div>
        )}
        ...
        <p className="mt-10 text-center text-sm text-slate-500">
          Die Bestellung erfolgt sicher &amp; unkompliziert über WhatsApp. Sofortige
          Lieferung deiner Zugangsdaten nach dem Kauf.
        </p>
      </div>
    </section>
  );
